...
首次初始化数据库时会创建管理员账号 `admin`，密码由 `ADMIN_INITIAL_PASSWORD` 环境变量指定。为了安全，登录页不会再展示任何默认账号或默认密码。

成员账号会随成员创建自动生成，初始密码仍为 `123456`，用户登录后可在左下角个人资料里自行修改。生产环境请务必修改管理员密码、数据库密码和 `SECRET_KEY`。
